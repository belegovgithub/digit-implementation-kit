...
ROLE_CODES = {"RO": "RO", "GRO": "GRO", "PGR-CE": "CSR", "TL Counter Employee": None,"CB Admin" :"STADMIN",
...
def loadDesig(df):
    ...
    for ind in df.index:
        row = df.iloc[ind] 
        designationData[row["Designation"]]=row["Code"]
...
def loadDept(df):
    ...
    for ind in df.index:
        row = df.iloc[ind] 
        departmentData[row["Department"]]=row["Code"]
    ...
    for root, dirs, files in os.walk(config.HRMS_CMD_FOLDER, topdown=True):
        for name in dirs:
            ...
            if os.path.exists(user_info_file)   :
                ...
                c = os.path.join(root, name).replace(os.path.join( config.HRMS_CMD_FOLDER,  "CB "),"").lower()
                print(c)
...
def main():
    ...
    df_dept = get_sheet(dfs,"Master - Department Master")
    departmentData = loadDept(df_dept)
    df_desig = get_sheet(dfs,"Master - Designation")
    designationData= loadDesig(df_desig)
    ...
    with io.open(os.path.join(config.HRMS_CMD_FOLDER,"user.csv") , mode="a+", newline="") as f:
        write = csv.writer(f) 
